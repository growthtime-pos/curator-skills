...
- Linux / macOS / WSL (글로벌 설치 시 심링크 사용)

## 설치 (OpenCode 글로벌)

```bash
git clone https://github.com/growthtime-pos/curator-skills.git ~/projects/curator-skills
cd ~/projects/curator-skills
bash scripts/install.sh
```

`scripts/install.sh`는 다음 두 심링크를 만듭니다.

| 글로벌 위치 | 원본 |
|---|---|
| `~/.config/opencode/skills/confluence-curation` | `<repo>/confluence-curation` |
| `~/.config/opencode/plugins/skill-update-check.js` | `<repo>/.opencode/plugins/skill-update-check.js` |

`OPENCODE_HOME` 환경변수로 prefix 변경 가능. `--uninstall`로 제거, `--force`로 덮어쓰기.

설치 후에는 OpenCode 세션이 시작될 때마다 plugin이 1회, 그리고 SKILL.md bootstrap 지시에 따라 LLM이 1회씩 `_skill_update_check.py`를 호출합니다(1시간 throttle 공유). 새 릴리즈 태그(`v*`)가 있고 작업 트리가 깨끗하며 `main` 브랜치라면 ff-only pull로 자동 갱신합니다. 그 외(다른 브랜치, dirty, fast-forward 불가, 오프라인)에는 stderr 알림만 남기고 정상 진행합니다.

리포 위치를 옮긴 경우 `bash scripts/install.sh --force`를 다시 실행해 심링크를 갱신하세요.
