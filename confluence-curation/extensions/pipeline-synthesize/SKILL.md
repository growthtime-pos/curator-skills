---
name: pipeline-synthesize
description: Convert evidence packs into topic-level conclusions and actions with a selectable synthesis strategy.
---

# Pipeline Synthesize

`stage4_synthesize` 는 evidence pack 을 사람이 읽을 결론과 action 으로 압축하는 skill stage다.

## Methods

- `balanced-synthesis`
- `briefing-synthesis`
- `action-heavy-synthesis`
- `evidence-first-synthesis`
- `pyramid-synthesis`
- `hypothesis-driven-synthesis`

컨설팅형 사고 프레임을 직접 고르고 싶다면 다음을 우선 고려한다.

- `evidence-first-synthesis`: 현재/배경/공백을 보수적으로 정리
- `pyramid-synthesis`: 답 먼저, 핵심 근거, wider significance
- `hypothesis-driven-synthesis`: 가설, 검증 포인트, pivot question

## Current Implementation

- runner: `scripts/synthesize_insights.py --strategy <method> --purpose <purpose>`
